...
def cloneRepo(url, path, branch='master'):
    """
       Use gitpython to clone the specified repo/branch if not already present
       at path.
    """
...
def cisToFBP(cismodel):
    """
    Convert dict from cisrun format to flow-based-protocol (FBP) format
    required by the-graph UI component.
    """
    fbpmodel = {}
...
def loadSpecs(repo, path):
    """ 
    Load model specs from the specified temporary path, converting 
    from the cisrun YAML to the flow-based-protocol format required for
    UI.  The "content" nested dict is a convention used for storing
